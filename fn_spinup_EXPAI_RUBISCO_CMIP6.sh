...
# make sure the elm will use CMIP6 protocol 
...
CurDate=`date '+%Y-%m-%d'`
RunName=fn_spinup_EXPAI_RUBISCO_CMIP6
...
./xmlchange STOP_N=210
./xmlchange REST_N=30
./xmlchange RESUBMIT=10


# change co2 concentration only for spinup case to be consistent with cmip6 value
# default is 284.7
./xmlchange CCSM_CO2_PPMV=284.207   
...
&elm_inparm
    ...
    hist_fincl1 = 'PPOOL','EFLX_LH_TOT','RETRANSN','PCO2','PBOT','NDEP_TO_SMINN','OCDEP','BCDEP','COL_FIRE_CLOSS',
                  'HDM','LNFM','NEE','GPP','FPSN','AR','HR','MR','GR','ER','NPP','TLAI',
                  'SOIL3C','TOTSOMC','TOTSOMC_1m','LEAFC','DEADSTEMC','DEADCROOTC','FROOTC',
                  'LIVESTEMC','LIVECROOTC','TOTVEGC','N_ALLOMETRY','P_ALLOMETRY','TOTCOLC','TOTLITC',
                  'BTRAN','SCALARAVG_vr','CWDC','QVEGE','QVEGT','QSOIL','QDRAI','QRUNOFF','FPI','FPI_vr',
                  'FPG','FPI_P','FPI_P_vr','FPG_P','CPOOL','NPOOL','PPOOL','SMINN','HR_vr','SOIL4C'
    hist_dov2xy = .true.
...
#-xum do not work by changing it in usr_nl_elm    co2_ppmv = 284.207
