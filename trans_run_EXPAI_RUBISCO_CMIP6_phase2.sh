...
CurDate=`date '+%Y-%m-%d'`
RunName=trans_EXPAI_RUBISCO_CMIP6_phase2
...
Transph2Case=${CurDate}.${RunName}.${Compset}.${GridRes}.${Machine}.${Compiler}_${Ens}
#Transph2Case=${CurDate}.${RunName}.${Compset}.${GridRes}.${Machine}.${Compiler}


Base_runDir=$BaseSimuDir/${Transph2Case}/run
Base_bldDir=$BaseSimuDir/${Transph2Case}/bld
...
# ==case create
$E3SMCimeDir/create_newcase --case $BaseCaseDir/$Transph2Case --mach ${Machine} --compset ${Compset} --res ${GridRes} --mpilib mpich\
             --walltime 24:00:00 --handle-preexisting-dirs u --project m2467 --compiler ${Compiler}


cd $BaseCaseDir/$Transph2Case
...
./xmlchange RUN_REFDIR=$Base_ph1Dir
./xmlchange RUN_REFCASE=$Transph1Case
./xmlchange RUN_REFDATE=1901-01-01    # no use for branch
./xmlchange RUN_STARTDATE=1901-01-01  # no use for branch
...
cat <<EOF >user_nl_datm
  ...
  taxmode = "cycle", "extend", "extend", "cycle", "cycle", "extend"
EOF
